...
## 1. 개요 (Overview)
본 과제는 ATmega128 마이크로컨트롤러로 PSD를 제어해 거리를 측정하는 것이 목표이다.
...
3. 계산된 거리 데이터를 Uart로 PC 시리널 터미널에서 출력한다.
4. 측정 주기 설정 및 비정상 센서 데이터는 예외처리한다.(너무 가깝거나, 너무 멀 때)
### 동작 사진 / 영상

| 정면 동작 모습 | 센서 측정 및 시리얼 출력 |
| :---: | :---: |
[Hardware Setup](https://drive.google.com/file/d/1wJDznRoz-d1WCjyee3osOOJAgpkyepVz/view?usp=drive_link)
---
...
2. **학습 주도성:** PSD가 물체랑 너무 가까이 있으면 오히려 값이 역전되어 PSD값이 작게 나올 수 있다. <br> AVR에는 부동소수점 전용 하드웨어가 없기 때문에 float을 사용하는것이 비효율적이다. 그래서 그냥 정수형 변수에서 따로 계산해 소수를 표현하는것이 효율적이다.
